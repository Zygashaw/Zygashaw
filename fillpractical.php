      ...
      <table class="table table-stried table-border table-hover">
      	...
      	<tbody>
      		...
      		<form action="" method="post">
				<?php
					if(isset($_POST["search"]))
					{
					  ...
					  $search2=$_POST["search2"];
						...
						$sql="SELECT * from applicantdoc where orgname='$search1' AND jobtitle='$search2' AND status='Approved' 
						AND status1='-'";
			    ...
			    ?>
						...
						<tr>
							...
							<td><input name="practical[]" type="text" required style="width:60px; height:25px;"/></td>
		      ...
		      <?php
				...
				if (!empty($_POST['practical']) && is_array($_POST['practical']) && !empty($_POST['test']) && is_array($_POST['test']))
				{

					$name_practical = $_POST['practical'];
				    ...
				    for ($i = 0; $i < count($name_aid); $i++) {
				        ...
				        $practical = mysql_real_escape_string($name_practical[$i]);
				        $testdate = mysql_real_escape_string($name_testdate[$i]);

				        $ftotal=$practical + $test;
				        ...
				        if($sex=="Female")
       	   			    ...
       	   			    if($ftotal >= 45)
						        ...
						        $sql=mysqli_query($con,"INSERT INTO applicantresult (aid,afullname,organaization_name,vacancy_title,sex,testdate,practical,test2,total_grade,status,eid_fk)
						                        VALUES('$id','$fullname','$orgname','$jobtitle','$sex','$testdate','$practical','$test','$ftotal','Selected For interview','$eid')") or die(mysqli_error($con));

						        $sql1= mysqli_query($con,"UPDATE applicantdoc SET status1='Tested' WHERE aid='$id'");
						        if($sql){

						        echo "<script> alert('Practical Test Filled successfully!') </script>";
                                echo "<script> window.open('fillpractical.php','_self') </script>";
                                   }
						        else 
						    	echo "<script> alert('Practical Test Not Filled') </script> " or die(mysqli_error($con));
						    }
						    

					    else
					    {
					    	$sql2=mysqli_query($con,"INSERT INTO applicantresult (aid,afullname,organaization_name,vacancy_title,sex,testdate,practical,test2,total_grade,status,eid_fk)
					                        VALUES('$id','$fullname','$orgname','$jobtitle','$sex','$testdate','$practical','$test','$ftotal','Not Selected For interview','$eid')") or die(mysqli_error($con));
